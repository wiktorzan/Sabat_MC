...
// If there is no hit collection, there is nothing to be done
	...
	G4int FlagAlpha=0, FlagNeu=0;
	    ...
	    // Coordinates axis after postStep
	  ...
	  //  if(i1==(numberHits-1) && totEdep>0) analysis->FillNtupleDColumn(0,totEdep);
            analysis->AddNtupleRow();
	    if( (energy_step>14.1) || (hit->GetKEnergy()>14.1) )
	    {G4cout<<" Fishy event = "<<evnt->GetEventID()<<G4endl;tempEvent=evnt->GetEventID();}
	    }
        
	    if(totEdep>0) analysis->FillNtupleDColumn(16,totEdep);
	    if(tempEvent==evnt->GetEventID())G4cout<<"////////////////////////////////////////////"<<G4endl;
